...
PP ClosestPair(vT &X_coord, vT &Y_coord){
    int n = sz(X_coord);
    Points X(n), Y(n);
    for(int i=0; i<n; ++i)
...
void solve(int n){
    ...
    PP ans = ClosestPair(X_coord, Y_coord);
    T dist = sqrt(dist2(ans.first, ans.second));
    if(dist < 1e4)  cout << fixed << setprecision(4) << dist << '\n';
    else            cout << "INFINITY\n";
}
